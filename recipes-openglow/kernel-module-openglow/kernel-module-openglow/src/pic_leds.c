...
static void pic_led_set_work(struct work_struct *work)
{
  ...
  pic_write_one_register(to_i2c_client(led_work->self->dev), led_work->reg, led_work->value);
  kfree(led_work);
}


static void pic_led_set(struct i2c_client *client, enum pic_register reg, enum led_brightness brightness)
{
...
static enum led_brightness pic_led_get(struct i2c_client *client, enum pic_register reg)
{
  return pic_read_one_register(client, reg);
}
